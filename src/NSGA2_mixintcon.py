...
def optimization(model, nInput, nOutput, xlb, xub, intstart, discrete, pop, gen, \
    ...
    constr = np.zeros(pop)
    for i in range(pop):
        y[i,:], constr[i] = model.evaluate(x_ori[i,:])
    icall = pop
    ...
    for i in range(gen):
        ...
        for k in range(pop):
            ...
            y1, constr1 = model.evaluate(child_o)
            x  = np.vstack((x,child))
            y  = np.vstack((y,y1))
        ...
        population_con = population_con[idx]
    ...
    idxp = (ranktmp == 0)
    bestx = population_para[idxp,:]
    besty = population_obj[idxp,:]
    bestc = population_con[idxp]

    return bestx, besty, bestc, x, y, constr 
...
def fast_non_dominated_sort(Y,constr):
    ...
    for i in range(N):
        for j in range(N):
            if i != j:
                if constr[i] == 0.0 and constr[j] == 0.0: 
