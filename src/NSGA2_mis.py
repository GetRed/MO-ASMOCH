...
# mixed-integer version for MOASMO_mixint
...
def optimization(model, nInput, nOutput, xlb, xub, intstart, pop, gen, \
    ...
    return bestx, besty, x, y, ranktmp
...
def fast_non_dominated_sort(Y):
    ...
    S = [] # temp array of Sp
    rank = np.zeros(N, dtype = np.int) # Pareto rank
...
def crowding_distance(Y):
    ...
    J = np.argsort(idx, axis=0, kind='stable')
